...
return [
    ...
    "CHUNK_SIZE"          => 0.5 * 1000 * 1000, # 上传时的分块大小（B），默认为1M，越大传输越快，需要小于web服务器和php.ini中设置的上传限值
    ...
    "FILE_SUB_DIR"        => @date("Y-m-d", time()), #资源文件目录的子目录生成规则，变量或常量均可
    ...
    "GROUPS"              => [ # 分组，可设置多个不同分组，各自拥有独立配置
        "file" => [ # 新增分组请尽量使用video、audio等有意义的分组名
            ...
            "FILE_EXTENSIONS"              => "MP4,TS,PS,AVI,WMV,ASF,RM,RMVB,MOV,FLV,F4V,MKV,MXF,3GP,GXF,HLS,".
                                                 "mp4,ts,ps,avi,wmv,asf,rm,rmvb,mov,flv,f4v,mkv.mxf,3gp,gxf,hls,".
                                                 "DTS,DTS-HD,MP1,MP2,MP3,WMA,AAC,AC3,AMR,RM,OGG,".
                                                 "dts,dtshd,mp1,mp2,mp3,wma,aac,ac3,amr,rm,ogg", # 被允许的资源文件扩展名，空为不限制，多个值以逗号分隔
